...
이 프로젝트는 dispatcher -> [calculate_node, read_file_node, write_file_node]
(팬아웃) -> reporter (팬인) -> END 구조를 쓴다. dispatcher가 tool_call을
하나만 냈어도 세 노드를 모두 리스트로 반환해서 "항상 셋 다 병렬로 깨우고,
자기 담당이 아니면 조용히 통과한다"는 원칙을 지킨다 —
add_edge(FANOUT_TOOL_NODES, "reporter") 형태의 팬인 조인은 세 노드가 매번
함께 실행되어야만 데드락 없이 동작하기 때문이다.

dispatcher -> 팬아웃 -> reporter -> END는 사이클이 없는 구조라(reporter는
도구가 바인딩되지 않은 llm을 쓰므로 구조적으로 다시 tool_call을 낼 수 없다),
이전에 있었던 "반복 상한(iteration cap)" 개념은 더 이상 필요 없다 — 그래프
자체가 무한 루프를 만들 수 없는 DAG이기 때문이다.
"""
...
def test_routes_to_all_fanout_tool_nodes_regardless_of_which_tool_was_called():
    ...
    # 팬인 조인(add_edge(FANOUT_TOOL_NODES, "reporter"))이 데드락 없이
    # 동작한다. calculate_node는 담당 tool_call이 없으면 그냥 통과한다.
    state = {"messages": [_ai_message_with_tool_call("read_sandbox_file")], "iteration": 1}
    assert route_after_dispatcher(state) == FANOUT_TOOL_NODES


def test_routes_to_end_when_no_tool_call():
    # 도구가 전혀 필요 없으면 reporter를 거치지 않고 dispatcher의 답변이
